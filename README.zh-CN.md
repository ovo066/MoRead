...
- 自绘排版引擎：经典仿真 / 现代仿真 / 覆盖 / 滑动翻页；原生 EPUB 精细解析 CSS、块级/行内、浮动、表格、背景与图片；支持选词拖柄、段落批注与评论、书签、书内关键词搜索
- EPUB 排版：支持从右向左的竖排、竖排标点、选词与翻页，以及受支持的 CSS 渐变、边框、文字阴影和定位元素
- 快捷书签：翻页模式下拉至提示后松手添加书签，重复下拉不会移除已有书签；上下连续滚动模式不启用此手势
- 排版自由：字号、行距、页边距、明暗主题、自定义三色阅读主题；无封面书籍自动生成直排文字封面
- 阅读工具：本地 MDX / MDD 词典、划线查词、生词本及可选中英对照、单段翻译、词下释义与音标；自定义点击区域、物理翻页按键、标题样式与渐变或图片高亮，正文净化可先预览后应用
- 划线与笔记回顾：按书籍、角色和内容筛选，以卡片或全屏方式翻阅，邀请伴读点评、共创，并使用可复用的分享模板导出
...
- 大屏适配：自适应书架网格、设置与会话侧栏、书籍详情与听书分栏，可选双页阅读、阅读与伴读并排显示
...
- 插图工作室：按书保存画风，上传人物参考图或生成三视图候选，按章节保存形象版本并复用生图配方；独立 DIY 页面可用描述、参考图和高级参数创建可跨书复用的画风模板；先预览章节计划，再批量生成已读范围内的插图，支持暂停、继续和失败重试
- 生图服务：支持 OpenAI 生图与图生图、chat 端点出图，以及对应 NovelAI 模型的角色参考与 Vibe 功能；参考图能力与一致性效果取决于所选服务和模型
- 语音与音色设计：支持系统引擎、MiniMax、OpenAI 兼容端点和 Gemini TTS；可浏览可用音色、试听自定义 Gemini 音色并保存到音色库；生图与语音均可独立于模型分配单独配置
...
应用界面目前仍以中文为主。已建立 Android 字符串资源与应用内语言切换（设置 → 阅读与外观 → 界面语言；Android 13+ 也可在系统的应用语言设置中切换），英文目前覆盖导航、设置首页及部分设置页、书籍长按菜单、书签与导入提示等，尚不是完整英文界面。翻译约定与贡献方法见 [本地化指南](docs/LOCALIZATION.md)。
