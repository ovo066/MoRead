...
internal class AddAnnotationTool(
    ...
    override suspend fun execute(arguments: JsonObject): ToolResult = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Default) {
        try {
            ...
            ToolResult.Failure(error.code, "正文核验失败（${error.code}），未写入批注，请重新读取原文。", error)
        } catch (error: Exception) {
            ToolResult.Failure("WRITE_FAILED", "正文核验或批注保存失败，请重新读取并确认批注列表，不能视为已成功添加。", error)
        }
    }

    private suspend fun executeChecked(args: JsonObject): ToolResult {
        fun text(key: String) = (args[key] as? JsonPrimitive)?.takeIf { it.isString }?.content
        val reference = text("source_ref")
        if ("source_ref" in args && reference.isNullOrBlank()) return ToolResult.Failure("INVALID_REFERENCE", "source_ref 无效，请重新 grep_book。")
        ...
        if (quote.isEmpty()) return ToolResult.Failure("INVALID_ARGUMENT", "缺少原文 quote")
        if (comment.isEmpty()) return ToolResult.Failure("INVALID_ARGUMENT", "缺少批注 comment")
        if (quote.length > 2_000) return ToolResult.Failure("INVALID_ARGUMENT", "quote 过长，请选择 2000 字以内的连续原文")
        val style = AnnotationStyle.fromWire(text("style"))
        val book = getBook() ?: return ToolResult.Failure("SOURCE_MISSING", "未找到当前书籍")
        if (book.totalChapters <= 0) return ToolResult.Failure("SOURCE_UNAVAILABLE", "规范正文未就绪，未写入批注。")
        val scope = readingScope.intersect(currentScope())
        val chapterNumber = (args["chapter_number"] as? JsonPrimitive)?.intOrNull
        if ("chapter_number" in args && chapterNumber == null) return ToolResult.Failure("INVALID_ARGUMENT", "chapter_number 必须是整数。")
        val last = scope.clampLastChapter(book.totalChapters)
        if (chapterNumber != null && chapterNumber !in 1..last + 1) return ToolResult.Failure("OUT_OF_SCOPE", "章节超出当前可见范围。")
        val revision = getRevision()
        val match: QuoteLocation
        if (reference != null) {
            val source = registry.source(reference) ?: return ToolResult.Failure("INVALID_REFERENCE", "source_ref 无效或已过期，请重新 grep_book。")
            if (chapterNumber != null && chapterNumber - 1 != source.chapterIndex) return ToolResult.Failure("INVALID_REFERENCE", "source_ref 与 chapter_number 不一致。")
            if (source.bookId != bookId || source.revision != revision ||
                !scope.allowsChunk(source.chapterIndex, source.start, source.end)) {
                return ToolResult.Failure("INVALID_REFERENCE", "source_ref 的书籍、版本或阅读范围已失效，请重新 grep_book。")
            }
            ...
            if (chapter == null || chapter.chapterIndex != source.chapterIndex || chapter.readError != null) {
                return ToolResult.Failure("SOURCE_UNAVAILABLE", "来源正文不可读，未写入批注。")
            }
            match = registry.verify(source, bookId, revision, scope, chapter.body, quote)
                ?: return ToolResult.Failure("INVALID_REFERENCE", "source_ref 与原始 quote 或区间不一致，未写入批注。")
        } else {
            val matches = ArrayList<QuoteLocation>()
            var scanned = 0
            val indexes = chapterNumber?.let { (it - 1)..(it - 1) } ?: 0..last
            if (indexes.count() > MAX_LEXICAL_CHAPTERS) return ToolResult.Failure("RESOURCE_LIMIT", "范围超过核验上限，请先 grep_book 并提供 source_ref。")
            for (index in indexes) {
                ...
                if (chapter == null || chapter.chapterIndex != index || chapter.readError != null) {
                    return ToolResult.Failure("SOURCE_UNAVAILABLE", "正文覆盖不完整，不能确认唯一位置；请先 grep_book 并提供 source_ref。")
                }
                val readable = scope.readableText(index, chapter.body)
                scanned += readable.length
                if (scanned > MAX_LEXICAL_CHARS) return ToolResult.Failure("RESOURCE_LIMIT", "范围超过核验上限，请先 grep_book 并提供 source_ref。")
                matches += locateExactQuote(listOf(chapter.copy(body = readable)), quote, maxMatches = 2)
                if (matches.size > 1) return ToolResult.Failure("AMBIGUOUS_QUOTE", "这段 quote 在已读范围出现多次，无法确定位置；请提供 chapter_number、唯一引文或 grep_book 的 source_ref。")
            }
            match = matches.singleOrNull() ?: return ToolResult.Failure("QUOTE_NOT_FOUND", "已读原文中找不到这段 quote；请从 read_book_section/search_book/grep_book 逐字复制。")
        }
        if (getRevision() != revision) return ToolResult.Failure("CONTENT_CHANGED", "正文版本已变化，未写入批注，请重新读取原文。")
        ...
        if (!finalScope.allowsChunk(match.chapterIndex, match.startCharOffset, match.endCharOffset)) {
            return ToolResult.Failure("SCOPE_CHANGED", "阅读范围已缩小，未写入批注。")
        }
        currentCoroutineContext().ensureActive()
        val id = save(match, quote, comment.take(10_000), style, finalScope)
        return ToolResult.Success("已在第 ${match.chapterIndex + 1} 章添加段落批注（编号 $id，样式 ${style.wire.lowercase()}），" +
