...
class BookLayoutStore @Inject constructor(
    ...
    suspend fun replace(
        ...
        try {
            ...
            val refs = sortedChapters.map { input ->
                require(input.document.schemaVersion == EpubLayoutPackage.CURRENT_SCHEMA_VERSION) {
                    "EPUB 章节布局版本不受支持：${input.chapterIndex}"
                }
                require(input.document.textLength >= 0) { "EPUB 章节布局长度无效：${input.chapterIndex}" }
                val fileName = "ch-${input.chapterIndex.toString().padStart(5, '0')}.json"
                File(chapterDirectory, fileName).writeText(
                    json.encodeToString(input.document.copy(chapterIndex = input.chapterIndex, href = input.href))
                )
            ...
            extractResources(epubFile, storedPackage, File(staging, RESOURCE_DIRECTORY))
    ...
    suspend fun readChapter(bookId: Long, chapterIndex: Int): EpubLayoutChapterBundle? =
        withContext(Dispatchers.IO) {
            ...
            if (layoutPackage.schemaVersion != EpubLayoutPackage.CURRENT_SCHEMA_VERSION) {
            ...
            if (chapter.schemaVersion != EpubLayoutPackage.CURRENT_SCHEMA_VERSION ||
                chapter.chapterIndex != chapterIndex || chapter.textLength != reference.textLength
            ) {
                return@withContext null
            }
            val paths = buildMap {
            ...
            EpubLayoutChapterBundle(chapter, paths, fonts, fontFaces)
        }
    ...
    suspend fun hasCurrentLayout(bookId: Long, expectedTextLengths: List<Int>): Boolean =
        withContext(Dispatchers.IO) {
            ...
            val index = runCatching { readPackageBlocking(bookId) }.getOrNull()
            ...
            expectedTextLengths.indices.all { chapterIndex ->
                ...
                val chapter = chapterFile.takeIf(File::isFile)?.let { file ->
                    runCatching {
                        json.decodeFromString(EpubLayoutChapter.serializer(), file.readText())
                    }.getOrNull()
                } ?: return@all false
                chapter.schemaVersion == EpubLayoutPackage.CURRENT_SCHEMA_VERSION &&
                    chapter.chapterIndex == chapterIndex && chapter.textLength == reference.textLength
            }
        }
    ...
    private fun extractResources(epubFile: File, layoutPackage: EpubLayoutPackage, targetRoot: File) {
        targetRoot.mkdirs()
        ZipFile(epubFile).use { zip ->
            ...
            layoutPackage.resources.asSequence()
                .filter { it.kind != EpubLayoutResourceKind.OTHER }
                .forEach { resource ->
