...
class AddAnnotationToolTest {
    ...
    @Test fun repeatedQuoteOnlyAnnotatesTheSpecifiedRange() = runTest {
        val result = tool().execute(args(source())).content
    ...
    @Test fun literalGrepToAnnotationWorksWithNormalizedOriginalAndWhitespace() = runTest {
        ...
        val result = Json.parseToJsonElement(grep.execute(buildJsonObject { put("pattern", " A123 ") }).content).jsonObject
        ...
        assertTrue(tool().execute(args(ref, quote)).content.contains("已在第"))
    ...
    @Test fun forgedOrWrongBookReferenceNeverFallsBackToFirstQuote() = runTest {
        assertTrue(tool().execute(args("s1.forged")).content.contains("无效"))
        tool().execute(args(source(bookId = 2))).content
    ...
    @Test fun aDifferentQuoteCannotUseTheReference() = runTest {
        tool().execute(args(source(), "伪造的引文")).content
        assertTrue(writes.isEmpty())
    }
    ...
    @Test fun scopeAndVersionAreRecheckedImmediatelyBeforeWrite() = runTest {
        val ref = source()
        tool { scope = ReadingScope.upto(0, 0); ChapterDocument(it, "", body) }.execute(args(ref)).content
        assertTrue(writes.isEmpty())
        scope = ReadingScope.WholeBook
        tool { revision = "v2"; ChapterDocument(it, "", body) }.execute(args(ref)).content
        assertTrue(writes.isEmpty())
    }

    @Test fun oldUniqueQuoteCallsRemainCompatibleAndAmbiguityIsRejected() = runTest {
        assertTrue(tool().execute(args()).content.contains("多次"))
        assertTrue(writes.isEmpty())
        body = "这是一段唯一的原文。"
        assertTrue(tool().execute(args(quote = "一段唯一的原文")).content.contains("已在第"))
        assertEquals(2, writes.single().startCharOffset)
    }

    @Test fun missingCanonicalSourceDoesNotWrite() = runTest {
        tool { null }.execute(args(source())).content
        assertTrue(writes.isEmpty())
    }
}
